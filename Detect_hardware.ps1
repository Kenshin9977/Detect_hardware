...
##
##Kd3HDZOFADWE8uO1
##Nc3NCtDXTlCDjpba9Dp76krqAksjfMqWvIr1lNDksrq6hybRQIpaQFd49g==
...
##P8HPCZWEGmaZ7/K1
##L8/UAdDXTlCDjuObowJ+4egnTXsUT9CfsLm0+K6ezNbKnSjJTJ0rbG5LsizoAUipGdccVvoRscMtcRg+KvEf3brRCeO+RLAP3OZnboU=
##Kc/BRM3KXhU=
##
##
...
for ($i=1;$i -lt $gpus.length; $i++) {
	...
	if ($gpus[$i] -match "^NVIDIA (Quadro GP100|(P100|V100) GPU Accelerator)( \d+GB)?$")
		{$nvenc_gen_4 = $true}
	if ($gpus[$i] -match "^NVIDIA (GeForce (GTX (1050|1050 Ti|1060|1070|1070 Ti|1080|1080 Ti))|Quadro (P1000|P2000|P400|P4000|P5000|P600|P6000)|TITAN (X|Xp|V)|(P4|V100) GPU Accelerator)( \d+GB)?$")
		{$nvenc_gen_5 = $true}
...
if($nvenc_gen_1 -or $nvenc_gen_2 -or $nvenc_gen_3 -or $nvenc_gen_4 -or $nvenc_gen_5){$nvenc_capable = $true}
if($nvenc_gen_1){"NVENC Gen1"} elseif($nvenc_gen_2){"NVENC Gen2"} elseif($nvenc_gen_3){"NVENC Gen3"} elseif($nvenc_gen_4){"NVENC Gen4"} elseif($nvenc_gen_5){"NVENC Gen5"}
if($nvenc_capable){"NVENC Capable, list of the encoding capacities :`nH.264 YUV 4:2:0"}
if($nvenc_gen_2 -or $nvenc_gen_3 -or $nvenc_gen_4 -or $nvenc_gen_5){"H.264 YUV 4:4:4"}
if($nvenc_gen_3 -or $nvenc_gen_4 -or $nvenc_gen_5){"H.264 Lossless"}
if($nvenc_gen_4 -or $nvenc_gen_5){"H.265 (HEVC) 4K Lossles"}
if($nvenc_gen_5){"H.265 (HEVC) 8K"}
...
if ($qsv_capable -and (-not ($qsv_gen_1 -or $qsv_gen_2 -or $qsv_gen_3 -or $qsv_gen_4 -or $qsv_gen_5))){"CPU has an iGP which is QuickSync capable but the iGPU isn't enabled."}
...
if($vce_capable -and (-not ($vce -or $vce_hevc))){"CPU has an iGP which is VCE capable but the iGPU isn't enabled."}
